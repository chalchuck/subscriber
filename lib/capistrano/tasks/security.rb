namespace :security do
	...
	task :install do
		on roles (:all) do
			execute :sudo ,"DEBIAN_FRONTEND=noninteractive apt-get install -y ufw fail2ban logcheck logcheck-database"
		end
	end
	...
	task :setup do
		on roles (:all) do
			...
			execute :sudo, "ufw allow 8080"
			execute :sudo, "ufw allow 8088"
			execute :sudo, "ufw allow 8089"
			execute :sudo, "ufw allow 8310"
		end
	end
